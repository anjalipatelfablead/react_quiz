...
const Sidebar = ({ isOpen, onClose }) => {
    ...
    return (
        <>
            ...
            {isOpen && (
                <div
                    className={`fixed inset-0 backdrop-blur-sm ${darkMode ? 'bg-black/40' : 'bg-white/30'} z-40 transition-all duration-300 lg:hidden`}
                    onClick={onClose}
                />
            )}
            ...
            <aside
                className={`fixed top-0 left-0 h-screen w-64 ${darkMode ? 'bg-gray-800 border-r border-gray-700' : 'bg-white shadow-xl'}  z-[60] transform transition-transform duration-300 ease-in-out 
                    lg:translate-x-0
            ...
            >
                ...
                <div className={`flex items-center justify-between p-4 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                    ...
                    <button
                        onClick={onClose}
                        className={`p-2 rounded-lg ${darkMode ? 'text-gray-400 hover:text-white hover:bg-gray-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'} transition-colors lg:hidden`}
                    ...
                    <div className={`p-4 border-b ${darkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <span className={`inline-block mt-1 px-2 py-0.5 text-xs rounded-full ${isAdmin ? (darkMode ? 'bg-purple-900/40 text-purple-300' : 'bg-purple-100 text-purple-600') : (darkMode ? 'bg-blue-900/40 text-blue-300' : 'bg-blue-100 text-blue-600')
                                    }`}>
                                    {user.role}
                                </span>
                            </div>
                        ...
                        {menuItems.map((item) => {
                            ...
                            return (
                                <li key={item.path}>
                                    ...
                                    >
                                        <Icon size={18} className={active ? 'text-orange-500' : (darkMode ? 'text-gray-500' : 'text-gray-400')} />
                ...
                {/* Sidebar Footer */}
                <div className={`absolute bottom-0 left-0 right-0 p-4 border-t ${darkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white lg:bg-gray-50'}`}>
